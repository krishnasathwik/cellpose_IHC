...
#     # ===============================
#     # 🔥 NEW: CREATE UNION MASK
#     # ===============================
#     union_mask = np.zeros_like(mask_preproc, dtype=np.uint8)

#     union_mask[mask_preproc > 0] = 1
#     cv2.drawContours(union_mask, unique_cv_contours, -1, 1, thickness=cv2.FILLED)

#     # ---- SAVE MASK ----
#     np.save(os.path.join(mask_dir, filename.replace(".png", "_mask.npy")), union_mask)

#     # ---- ORIGINAL VISUALIZATION (UNCHANGED) ----
#     overlay_union = cv2.cvtColor(patch.copy(), cv2.COLOR_RGB2BGR)
#     overlay_union[union_mask > 0] = [0, 0, 255]

#     overlay_union_rgb = cv2.cvtColor(overlay_union, cv2.COLOR_BGR2RGB)

#     plt.imshow(overlay_union_rgb)
#     plt.title("Union Overlay")
#     plt.axis('off')

#     plt.savefig(os.path.join(target_dir, filename.replace(".png", "_union.png")))
#     plt.close()

#     print("Processed:", filename)

# print("\n✅ DONE — MASKS + VISUALS GENERATED")
# ===============================
